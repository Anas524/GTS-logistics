...
<section id="dh-sub-root"
    ...
    data-existing-names='@json($existingSubNames)' data-is-admin="{{ ($user = auth()->user()) && method_exists($user, 'isAdmin') && $user->isAdmin() ? 1 : 0 }}">
    <div class="max-w-6xl mx-auto font-plus space-y-6">
        ...
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white/80 px-6 py-10 text-center text-[11px] text-slate-500">
            ...
            @else
            <p class="mb-1">No subfolders are available yet.</p>
            @endif
        </div>
        @endif
    </div>
...
<script>
    $(function() {
        ...
        // If not admin, stop here – no create/delete JS
        if (!isAdmin) {
            return;
        }

        // ----- Admin-only: create + delete logic -----
